...
public class DelayedRestartTrigger extends Thread {
	...
	// 500 ms was to quick when using changing xtend files on my i5+SSD.
	private long settleDownMillis = 700;
	private final RunClassInSeparateJvmMachine runClassInSeparateJvmMachine;

	public final AtomicBoolean mute = new AtomicBoolean(false);
	...
	public void run() {
		...
		while (!shutdown) {
			try {
				...
				// wait for restarts requests to settle (e.g. accumlated file
				// changes)
				int totalTriggerCount = 0;
				do {

					log.debug("Delaying restart for " + settleDownMillis + " ms to wait for file changes to settle");
				...
				log.debug("Restarting SuperDevMode (" + totalTriggerCount + " file change(s) detected)");
				// log.info("------------------------------------------------------------------------");
	...
	/**
	 * Usually this method is called by an external component that watches a
	 * directory to restart Ninja's dev mode.
	 * 
