...
public class RunClassInSeparateJvmMachine {
	...
	private AtomicBoolean tobeignored = new AtomicBoolean(false);

	public RunClassInSeparateJvmMachine(String name, String classNameWithMainToRun, List<String> classpath,
			List<String> jvmArguments, File mavenBaseDir, boolean compile) {

		this(name, classNameWithMainToRun, StringUtils.join(classpath, File.pathSeparator), jvmArguments, mavenBaseDir,
				compile);

	}

	public RunClassInSeparateJvmMachine(String name, String classNameWithMainToRun, String classpath,
			List<String> jvmArguments, File mavenBaseDir, boolean compile) {
	...
	public synchronized void restart() {
		...
		try {
			...
			if (compile) {
				// Stop the listening process as the compilation
				// changes watched files
				tobeignored.set(true);
				CompileCommand.compile();
			}
			log.info("Restarting SuperDevMode (file changes detected)");
	...
	ProcessExecutor buildProcessExecutor() {
		...
		return new ProcessExecutor(commandLine).directory(mavenBaseDir).destroyOnExit()
				.addListener(new ProcessListener() {
					@Override
					public void afterStop(Process process) {
						if (!restarting.get()) {
							log.error("JVM process for {} terminated (next file change will attempt to restart it)",
									name);
						}
					}
	...
	public void setTrigger(DelayedRestartTrigger restartTrigger) {
