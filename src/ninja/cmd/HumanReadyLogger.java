...
public class HumanReadyLogger implements MessageLogger, TransferListener {
	...
	public void niceLog(String msg, int level) {
		try {
			...
			if (msg.startsWith("found ")) { // Depedency found
				...
				if (m.matches()) {
					println("| \t" + m.group(1) + "->" + m.group(2) + " " + m.group(3) + " (from " + m.group(4) + ")");
					return;
				}
			}
			...
			if (msg.startsWith("downloading ")) {
				if (downloading == null) {
					displayln("|");
					displayln("| Downloading required dependencies,");
					displayln("|");
				}
				downloading = msg;
				progress = 0;
				...
				displayln("| \t" + downloading);
				return;
			}

			if (msg.startsWith("[SUCCESSFUL")) {
				msg = msg.substring(msg.indexOf("("));
				displayln("\r| \t" + (downloading + "                    ").replace("(jar)", "")
						.replace("downloading", "downloaded").replace("...", "   "));
			}
	...
	private void displayln(String string) {
		System.out.println(string);
	}
	...
	public void transferProgress(TransferEvent te) {
		if (downloading != null) {
			length += te.getLength();
			print("\r| \t" + downloading.replace("...", "") + progressBar[progress]
					+ " " + String
							.format("%-20s",
									FileUtils.byteCountToDisplaySize(length) + (te.isTotalLengthSet()
											? "/" + FileUtils.byteCountToDisplaySize(te.getTotalLength()) : ""))
					+ "\r");
